...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT (books.title ) FROM books, characters INNER JOIN character_books ON books.id=character_books.book_id AND characters.id=character_books.character_id GROUP BY characters.name"
end
